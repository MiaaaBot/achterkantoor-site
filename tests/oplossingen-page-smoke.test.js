...
test('oplossingen page contains task 2 conversion sections', () => {
  ...
  assert.match(html, /<section[^>]+class="solutions-catalog section"/);
  ...
  assert.match(html, /<div[^>]+class="sticky-cta"[^>]+id="stickyCta"/);
});
...
test('solutions page JS keeps CTA hooks on rendered cards and avoids hidden injected panels', () => {
  ...
  assert.match(script, /data-solution-action="details"/);
